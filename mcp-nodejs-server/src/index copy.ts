...
const getServer = () => {
    ...
    server.tool(
        "getEmployeeCount",
        {
            title: "Récupère le nombre d'employés d'une entreprise",
            description: "Donne le nombre d'employés pour une entreprise donnée.",
        ...
        async ({ companyName }) => {
            ...
            const count = fakeDb[companyName] ?? 0;

            return {
                content: [
                    {
                        type: "text",
                        text: `L'entreprise "${companyName}" a ${count} employé(s).`,
                    },
                ],
            };
        }
